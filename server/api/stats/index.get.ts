...
export default defineEventHandler(() => {
  ...
  const bibGourmandCount = restaurants.filter((r) => r.distinction?.slug === "bib-gourmand").length;
  const countriesCount = new Set(restaurants.map((r) => r.country?.slug).filter(Boolean)).size;
